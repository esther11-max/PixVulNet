...
'''
Unlike train_model.py, this part has added data augmentation.
'''
...
def main():
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomRotation(10),  # 随机旋转，角度范围[-10, 10]
        transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2),  # 随机调整颜色
        transforms.ToTensor(),  # 转换为张量
        transforms.Lambda(lambda x: x.float())  # 确保数据类型为float
    ])
